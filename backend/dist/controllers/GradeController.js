...
class GradeController {
    ...
    async createOrUpdate(req, res) {
        const gradeRepository = database_1.AppDataSource.getRepository(Grade_1.Grade);
        ...
        try {
            ...
            // RF#25 - Calcular média automaticamente
            grade.average = this.computeAverage(grade.grade1, grade.grade2, grade.grade3);
            // RF#26 - Determinar situação do aluno
            grade.finalStatus = this.computeStatus(grade.average, grade.attendancePercentage);
            const savedGrade = await gradeRepository.save(grade);
            // Atualizar histórico acadêmico
            await this.updateAcademicHistory(disciplineEnrollment, savedGrade, academicHistoryRepository);
    ...
    async updateAttendance(req, res) {
        ...
        try {
            ...
            grade.attendancePercentage = (grade.attendedClasses / grade.totalClasses) * 100;
    ...
    async getAverage(req, res) {
        ...
        try {
            ...
            const average = this.computeAverage(grade.grade1, grade.grade2, grade.grade3);
            return res.json({
                disciplineEnrollmentId,
                grade1: grade.grade1,
                grade2: grade.grade2,
    ...
    async findByDiscipline(req, res) {
        ...
        try {
            ...
            const grades = disciplineEnrollments.map((de) => ({
                ...de.grades,
    ...
    // Métodos auxiliares
    computeAverage(grade1, grade2, grade3) {
        const grades = [grade1, grade2, grade3].filter(g => g !== undefined && g !== null);
        if (grades.length === 0)
            return 0;
        const sum = grades.reduce((acc, g) => acc + g, 0);
        return Number((sum / grades.length).toFixed(2));
    }
    computeStatus(average, attendance) {
    ...
    async updateAcademicHistory(disciplineEnrollment, grade, academicHistoryRepository) {
        try {
            ...
            if (history) {
                ...
                history.status = this.mapStatus(grade.finalStatus);
    ...
    mapStatus(status) {
        switch (status) {
            case 'approved': return AcademicHistory_1.AcademicHistoryStatus.APPROVED;
            case 'failed': return AcademicHistory_1.AcademicHistoryStatus.FAILED;
            case 'recovery': return AcademicHistory_1.AcademicHistoryStatus.RECOVERY;
            case 'cancelled': return AcademicHistory_1.AcademicHistoryStatus.CANCELLED;
            default: return AcademicHistory_1.AcademicHistoryStatus.IN_PROGRESS;
        }
    }
}
